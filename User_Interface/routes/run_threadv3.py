...
done_feeding_event = threading.Event()

# ======================== Pick and Place Event ==============================

nozzle_pick_event = threading.Event()
nozzle_deliver_event = threading.Event()
done_pick_and_place_event = threading.Event()

FIRST_PICK_AND_PLACE = True
...
def _prefeedHose_loop():
    ...
    while True:
        ...
        if lubrication_feeder.close_hose_holder() == "success":
            ...
            if insertion_servos.holder_hose_nozzle_semi_close() == "success":
            
                if lubrication_feeder.move_feeder_until_ir(speed=290) == "success": 
                    pass
                else:
                    tmp_error = "ER-PF03"
            else :
...
def pickNozzle():
    ...
    global first_pick_after_align_nozzle
...
def pickAndDeliverJoint():

    global pick_and_place,pick_and_place_camera
    global first_pick_after_align_joint
...
def _movePickandPlace_loop():
    global PICK_AND_PLACE_ERROR

    while True:

        nozzle_pick_event.wait()
        nozzle_pick_event.clear()

        with PICK_AND_PLACE_ERROR_LOCK:
            PICK_AND_PLACE_ERROR = None
        ...
        if error is not None:
            with PICK_AND_PLACE_ERROR_LOCK:
                PICK_AND_PLACE_ERROR = error
            done_pick_and_place_event.set()
            continue

        nozzle_deliver_event.wait()
        nozzle_deliver_event.clear()
...
def oneCycle():
    ...
    done_pick_and_place_event.clear()

    with PICK_AND_PLACE_ERROR_LOCK:
        if PICK_AND_PLACE_ERROR is not None:
            return f"ER-{PICK_AND_PLACE_ERROR}"
    ...
    done_feeding_event.wait()
    done_feeding_event.clear()

    with FEED_ERROR_LOCK:
        if FEED_ERROR is not None:
            return f"ER-{FEED_ERROR}"

    #Lubricate Hose
    if insertion_servos.holder_hose_nozzle_close() != "success" : return "error13"
...
class CycleRunnerApp:
    ...
    def _poll_ui(self):
        ...
        if self.is_running or finished:
            ...
            nsteps = len(self.step_frames) if hasattr(self, "step_frames") else len(STEPS)

            if self.is_running:
                self._color_steps(step)
            elif finished:
                self._color_steps(nsteps)

            ts = total * nsteps
            ds = (cyc - 1) * nsteps + step if cyc > 0 else 0
            if finished:
                ds = ts
