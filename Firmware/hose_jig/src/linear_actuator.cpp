...
void LinearActuator::abs_mode(float angle, uint8_t* payload) {
    // Debug output
    Serial.print("abs_mode called with angle: ");
    Serial.println(angle, 6);
    
    // Convert angle to steps (45.51 steps/degree)
    int32_t steps = round(angle * angle_ratio);
    Serial.print("Raw steps: ");
    Serial.println(steps);
    
    // Handle negative angles by converting to 24-bit two's complement
    if (steps < 0) {
        steps = 0xFFFFFF + steps + 1;  // 0xFFFFFF is 2^24 - 1
        Serial.print("After two's complement: ");
        Serial.println(steps, HEX);
    }
    
    Serial.print("Final steps: 0x");
    Serial.println(steps, HEX);
    uint8_t byte1 = 0xF5;
