...
#define speedServo2_CCW_C  1450                                                                                 // Range from 500 to 1500   (the closer to 1500 the slower)
...
#define zeroPosition  800                                                                                     // Do not modify, this is to place the Cutter servo in Cut position
...
//Push Buttons//
////////////////
#define BUTTON_FULLCYCLE 12                                                                               // Push button for Full Cycle / Cut
...
void loop() 
{
    ...
    if (readingFullCycle != lastStableStateFullCycle)                                                         // Check if the reading is different from the last stable state
    {
      ...
      if (lastStableStateFullCycle == LOW && !sequenceRunning)                                                // Button pressed and no sequence running
      {
        ...
        if (fullCycleFirstPress) {
          Serial.println("FullCycle Case: First press - holder_home(), feed_tape()");
          holder_home(); // Ensure holder is in home position
          delay(100);
          feed_tape();
          delay(10);
          ...
          // Serial.println("FullCycle: elevator_up(), delay, gripper_close(), holder_home(), sync_move()");
          // holder_hold();
          // delay(100);
          // cut()
          // delay(100);
          ...
          // Serial.println("FullCycle: holder_home(), feed_tape(), holder_hold(), cut()");
          // holder_home(); // Ensure holder is in home position
          // delay(10);
          // feed_tape();
          // delay(200);
  ...
  if ((millis() - lastDebounceTimeForward) >= debounceDelayTest)                                              // If the debounce period has passed
  {
    if (readingForward != lastStableStateForward)                                                             // If the reading is different from the last stable state
    {
      ...
      if (lastStableStateForward == LOW && !sequenceRunning)                                                  // Button pressed and no sequence running
      {
        ...
        Serial.println("ButtonForward: elevator_up(), delay, gripper_close(), holder_home(), sync_move()");
        holder_hold();
        delay(100);
        elevator_up();
        delay(100);                                                                                           // Short delay to allow elevator to settle before closing gripper
  ...
  if ((millis() - lastDebounceTimeBackward) >= debounceDelayTest)                                             // If the debounce period has passed
  {
    if (readingBackward != lastStableStateBackward)                                                           // If the reading is different from the last stable state
    {
      ...
      {
        sequenceRunning = true;
        Serial.println("ButtonBackward: wrap_ccw(), wrap_cw(), cut(), holder_hold(), wrapper_3cw()");
        wrap_ccw();
        delay(10);
        wrap_cw();
        delay(10);
...
// Funciones Anteriormente llamadas STEP
...
void cut()
{
  ...
  servo3.writeMicroseconds(800);                                                                            // Wakeup pulse
  delay(300);                                                                                                  // 300 ms pause between movements
  servo3.writeMicroseconds(1850);                                                                            // Servo3 (cutter) moves to Cut position
  ...
  servo3.writeMicroseconds(800);                                                                            // Servo 3 returns to Home position
...
void holder_hold()
{
  ...
  servo4.writeMicroseconds(1850);                                                                    // Servo4 moves to Holding position
...
void holder_home()
{
  ...
  servo4.writeMicroseconds(1550);                                                                    // Servo4 moves to Home position
...
void gripper_close()
{
  ...
  servo5.writeMicroseconds(1850);                                                                    // Servo5 moves to Close position
...
void gripper_open()
{
  ...
  servo5.writeMicroseconds(2100);                                                                    // Servo5 moves to Open position
...
void elevator_up()
{
  ...
  gripper_open();                                                                                                     // Open gripper while elevator goes up
...
void sync_move() 
{
  ...
  {   
    ...
    if (!servo6Done)                                                                                            // If servo6 has not finished its movement
    {
      ...
      {
        currentAngle6 -= 1;                         
        delayMicroseconds(600); // 1.5 milisegundos        ? 1500                                                // Move servo6 down by 1 unit
...
void process_instruction(CanFrame instruction)
{
  ...
  switch (instruction.data[0])
  {
    ...
    case 0x10:
    {
      ...
      if (fullCycleFirstPress) {
        Serial.println("FullCycle: First press - holder_home(), feed_tape()");
        holder_home(); // Ensure holder is in home position
        delay(10);
        feed_tape();
        delay(10);
...
void send_twai_response(const byte response_data[8]) {
  ...
  // Print outgoing CAN frame details before sending
  Serial.print("TX CAN ID: 0x");
  Serial.println(tx_frame.identifier, HEX);
  Serial.print("TX DLC: ");
  Serial.println(tx_frame.data_length_code);
  Serial.print("TX DATA: ");
  for (int i = 0; i < 8; i++) {
    if (tx_frame.data[i] < 0x10) Serial.print("0");
