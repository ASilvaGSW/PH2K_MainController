...
void process_instruction(CanFrame instruction)
{
  ...
  switch (instruction.data[0])
  {
    ...
    // ***************************** CASE 0x1F ***************************** //
    // Move feeder with speed mode until IR sensor is triggered
    case 0x1F:
    {
      Serial.println("Case 0x1F: Moving feeder with speed mode until IR sensor triggered");
      ...
      uint8_t payload[8] = {0};  // Initialize buffer for CAN message
      feeder.speed_mode(dir, local_speed, acceleration, payload);  // Use speed mode directly
      ...
      if (CAN1.sendMsgBuf(feeder.motor_id, 0, 5, payload) != CAN_OK)
      {
        ...
        byte errorResponse[] = {0x1F, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};  // NO LOCAL NETWORK
        send_twai_response(errorResponse);
        break;
      }

      // Monitor IR sensor while moving
      bool hose_detected = false; // Starts as false, since we're waiting for it to be triggered
      unsigned long start_time = millis();
      const unsigned long timeout = 30000; // 30 second timeout

      while (!hose_detected && (millis() - start_time) < timeout)
      {
        // Read sensor (LOW = hose detected, HIGH = no hose)
        hose_detected = !digitalRead(IR_SENSOR_PIN); // Inverted logic for IR sensor
        
        if (hose_detected) {
          Serial.println("Hose detected - stopping feeder movement");
          
          // Stop the actuator by sending speed mode with 0 speed
          uint8_t stop_payload[8] = {0};
          feeder.speed_mode(false, 0, acceleration, stop_payload);
          
          if (CAN1.sendMsgBuf(feeder.motor_id, 0, 5, stop_payload) == CAN_OK) {
            Serial.println("Feeder stopped successfully");
          }
          break;
        }
      ...
      if (millis() - start_time >= timeout) {
        Serial.println("Timeout reached - stopping feeder movement");
        // Send stop command on timeout
        uint8_t stop_payload[8] = {0};
        feeder.speed_mode(false, 0, acceleration, stop_payload);
        CAN1.sendMsgBuf(feeder.motor_id, 0, 5, stop_payload);
      }
      ...
      byte statusResponse[] = {0x1F, status, (byte)hose_detected, 0x00, 0x00, 0x00, 0x00, 0x00};
      send_twai_response(statusResponse);
    }
    break;
